...
func (n *node32) Insert(key uint32, bits int, value uint8) *node32 {
	// Adjust bits.
	if bits < 0 {
		bits = 0
...
func (n *node32) InplaceInsert(key uint32, bits int, value uint8) *node32 {
	// Adjust bits.
	if bits < 0 {
		bits = 0
	} else if bits > key32BitSize {
...
func (n *node32) Match(key uint32, bits int) (uint8, bool) {
	// If tree is empty -
	if n == nil {
		// report nothing.
		return 0, false
...
func (n *node32) ExactMatch(key uint32, bits int) (uint8, bool) {
	// If tree is empty -
	if n == nil {
		// report nothing.
		return 0, false
...
func (n *node32) inplaceInsert(key uint32, bits uint8, value uint8) *node32 {
	var (
		p      *node32
		branch uint32
	)
...
func newNode32(key uint32, bits uint8, leaf bool, value uint8) *node32 {
	return &node32{
		key:   key,
		bits:  bits,
		leaf:  leaf,
