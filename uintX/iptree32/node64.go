...
// !!!DON'T EDIT!!! Generated by infobloxopen/go-trees/etc from domaintree{{.bits}} with etc -s uint32 -d uintX.yaml -t ./domaintree\{\{.bits\}\}
...
func (n *node64) Insert(key uint64, bits int, value uint32) *node64 {
	if bits < 0 {
		bits = 0
...
func (n *node64) InplaceInsert(key uint64, bits int, value uint32) *node64 {
	// Adjust bits.
	if bits < 0 {
		bits = 0
	} else if bits > key64BitSize {
...
func (n *node64) Match(key uint64, bits int) (uint32, bool) {
	if n == nil {
		return 0, false
	}
...
func (n *node64) ExactMatch(key uint64, bits int) (uint32, bool) {
	if n == nil {
		return 0, false
	}
...
func (n *node64) inplaceInsert(key uint64, bits uint8, value uint32) *node64 {
	var (
		p      *node64
		branch uint64
	)
...
func newNode64(key uint64, bits uint8, leaf bool, value uint32) *node64 {
	return &node64{
		key:   key,
		bits:  bits,
		leaf:  leaf,
