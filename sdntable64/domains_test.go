...
func TestDomainsMergeOnePass(t *testing.T) {
...
func TestDomainsRemoveOnMerge(t *testing.T) {
	...
	ds = ds.inplaceInsert(makeDomainNameFromString("02.example.com").GetComparable(), 0)
	assert.Equal(t, []int64{
		// 0 0 1+3   E L P M A X E 1+0   M O C 1+4
		0x303031, 0x454c504d41584501, 0x4d4f4341,
		// 2 0 1+3   E L P M A X E 1+0   M O C 1+4
	...
	assert.Equal(t, []uint64{0x01, 0x00, 0x20}, ds.data.v)
	assert.Equal(t, []uint32{0, math.MaxUint32, 10}, ds.data.i)
	...
	assert.Equal(t, []uint64{0x01, 0x20}, ds.data.v)
	assert.Equal(t, []uint32{0, 9}, ds.data.i)
